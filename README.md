...
- таблиця **`ai_models`** є persisted mirror/cache моделей Ollama з підтримкою історичних записів
- `GET /api/models` читає **тільки** з PostgreSQL і повертає лише активні моделі (`is_active = TRUE`)
...
backend/
  ...
  migrations/
    001_init.sql
    002_seed.sql
	003_add_ai_models_is_active.sql
	004_convert_created_at_to_timestamptz.sql
  internal/
    db/
      postgres.go
    handlers/
...
#### `backend/migrations/003_add_ai_models_is_active.sql`

Додає поле `is_active` до таблиці `ai_models`:
...
- дозволяє розділити:
    - актуальні моделі (доступні в Ollama)
    - історичні моделі (які більше не доступні, але використовуються старими задачами)
- використовується для того, щоб:
    - `GET /api/models` повертав тільки актуальні моделі
    - при цьому зберігалася цілісність історії задач

---

#### `backend/migrations/004_convert_created_at_to_timestamptz.sql`

Оновлює тип поля `created_at` у таблицях:

- `prompt_tasks`
- `transactions`

З `timestamp` → `timestamptz`

Призначення:

- забезпечує коректну роботу з часовими зонами
- гарантує, що всі дати зберігаються у timezone-aware форматі
- покращує узгодженість даних при роботі з різними клієнтами та середовищами
...
curl -X POST http://localhost:8080/api/tasks ^
  ...
  -d "{\"userId\":\"user-1\",\"modelId\":\"qwen2.5-1.5b\",\"payload\":\"Explain worker queues\"}"
```
